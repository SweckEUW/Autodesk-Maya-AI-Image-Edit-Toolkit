...
class FFTTensor(nn.Module):
    ...
    def fft_image(self, input, s=0):
        ...
        input = torch.view_as_real(torch.fft.fft(input, dim=s_dim)) # torch.rfft(input, signal_ndim=s_dim, onesided=False)
    ...
    def ifft_image(self, input, s=0):
        ...
        w = torch.stack((real, imaginary), -1)
        return torch.fft.irfft(torch.view_as_complex(w), n=w.shape[1], dim=s_dim) # torch.irfft(torch.stack((real, imaginary), -1), signal_ndim=s_dim, onesided=False)
